...
def CapacitorPlacasParalelas(largo, separacion, Dielectrico, ventana):
    ...
    if largo != 0:
        ...
        canvas_widget.place(x=625,y=80)
        plt.close()

def CapacitorEsferico(Ri, Re, Dielectrico, ventana):
    # Crear una figura y un eje
    fig = Figure(figsize=(5, 4), dpi=100)
    ax = fig.add_subplot(111)
    fig.patch.set_facecolor('#f0f0f0')
...
def CapacitorCilindrico(Ri, Re, largo, Dielectrico, ventana):
    # Crear una figura y un eje
    fig = Figure(figsize=(5, 4), dpi=100)
    ax = fig.add_subplot(111)
    fig.patch.set_facecolor('#f0f0f0')
